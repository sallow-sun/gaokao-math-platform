...
public class ProblemService {
  ...
  public PageResponse<ProblemListVO> query(ProblemQuery q, Long viewerUserId) {
    ...
    List<String> tags = taxonomy.expand(cleanStrings(q.tags()));
    List<String> types = cleanStrings(q.types());
    List<String> levels = cleanStrings(q.levels());
    validateCodes(types, levels);
    var learned = curriculum.validateCodes(q.learned());
    var chapters = curriculum.validateCodes(q.chapters());

    long total =
        problemMapper.countFiltered(
            clean(q.keyword()),
            q.years(),
            cleanStrings(q.sources()),
            types,
            levels,
            tags,
            tags.size(),
            ...
            learned,
            chapters);
    List<ProblemSummaryRow> rows =
        problemMapper.selectFiltered(
            clean(q.keyword()),
            q.years(),
            cleanStrings(q.sources()),
            types,
  ...
  public ProblemDetailVO detail(String problemNumber, Long viewerUserId) {
    ...
    if (Boolean.TRUE.equals(p.getDeleted())) throw BusinessException.notFound("PROBLEM_DELETED", "题目已删除");
    problemMapper.incrementViewCount(p.getId());
    p.setViewCount((p.getViewCount() == null ? 0 : p.getViewCount()) + 1);

    ProblemSource source =
        p.getSourceCode() == null ? null : sourceMapper.selectById(p.getSourceCode());
    ...
    if (p.getCreatorUserId() != null) {
      ...
      if (u != null)
        uploader =
            new UploaderVO(
                u.getPublicId().toString(), u.getUid(), u.getUsername(), u.getAvatarUrl());
    }
    List<ProblemAssetVO> assets =
        assetMapper.findByProblemId(p.getId()).stream()
            .map(
                a ->
                    new ProblemAssetVO(
                        a.getId().toString(),
    ...
    return new ProblemDetailVO(
        p.getProblemNumber(),
        title(p.getProblemNumber(), p.getTitle()),
        p.getRegion(),
        p.getYear(),
        p.getRegion(),
        p.getSourceCode(),
        sourceLabel,
        p.getQuestionType(),
        type == null ? p.getQuestionType() : type.label(),
        p.getDifficulty(),
        tags,
        sourceText(p.getYear(), sourceLabel, p.getRegion()),
        p.getContent(),
        p.getAnswer(),
        p.getSolution(),
        Optional.ofNullable(p.getContentFormat()).orElse("markdown-latex-v1"),
        new ProblemStatsVO(
            Optional.ofNullable(p.getViewCount()).orElse(0L),
            Optional.ofNullable(p.getFavoriteCount()).orElse(0L)),
        viewerState,
        uploader,
        assets,
        p.getCreatedAt(),
        p.getUpdatedAt(),
        curriculum.forProblem(p.getId()));
  }

  public ProblemCatalogVO catalog() {
    List<ProblemSource> sources =
        sourceMapper.selectList(
            new LambdaQueryWrapper<ProblemSource>()
                .eq(ProblemSource::getActive, true)
                .orderByAsc(ProblemSource::getSortOrder)
                .orderByAsc(ProblemSource::getCode));
    List<Tag> tags =
        tagMapper.selectList(
            new LambdaQueryWrapper<Tag>()
                .eq(Tag::getActive, true)
                .orderByAsc(Tag::getSortOrder)
                .orderByAsc(Tag::getId));
    return new ProblemCatalogVO(
        problemMapper.distinctYears(),
        sources.stream()
            .map(s -> new ProblemCatalogVO.SourceOption(s.getCode(), s.getLabel()))
            .toList(),
        Arrays.stream(QuestionType.values())
            .map(t -> new ProblemCatalogVO.Option(t.code(), t.label()))
            .toList(),
        Arrays.stream(DifficultyLevel.values())
            .sorted(Comparator.comparingInt(DifficultyLevel::rank))
            .map(d -> new ProblemCatalogVO.Option(d.code(), d.label()))
            .toList(),
        tags.stream().map(t -> new ProblemCatalogVO.TagOption(t.getName())).toList());
  }

  public ProblemCollectionStatsVO collectionStats() {
    return new ProblemCollectionStatsVO(
        problemMapper.selectCount(null),
        problemMapper.countDistinctYears(),
  ...
  public Map<Long, ProblemListVO> summariesByInternalIds(List<Long> ids, Long viewerUserId) {
    ...
    Map<String, String> sourceLabels =
        sourceMapper.selectList(null).stream()
            .collect(
                Collectors.toMap(ProblemSource::getCode, ProblemSource::getLabel, (a, b) -> a));
    rows.forEach(r -> r.setSourceLabel(sourceLabels.get(r.getSourceCode())));
    List<ProblemListVO> vos = buildList(rows, viewerUserId);
    Map<String, Long> byNumber =
        rows.stream()
            .collect(
                Collectors.toMap(ProblemSummaryRow::getProblemNumber, ProblemSummaryRow::getId));
  ...
  private List<ProblemListVO> buildList(List<ProblemSummaryRow> rows, Long viewerUserId) {
    ...
    var curricula = curriculum.forProblems(ids);
    Map<Long, List<ProblemAssetVO>> assets = new HashMap<>();
    List<ProblemAsset> problemAssets =
        assetMapper.selectList(
            new LambdaQueryWrapper<ProblemAsset>()
    ...
    for (ProblemAsset asset : problemAssets) {
      assets
          .computeIfAbsent(asset.getProblemId(), k -> new ArrayList<>())
          .add(
              new ProblemAssetVO(
    ...
    if (viewerUserId != null) {
      for (UserProblemState state : stateMapper.findMany(viewerUserId, ids))
        states.put(state.getProblemId(), state);
    ...
    for (ProblemSummaryRow r : rows) {
      ...
      if (viewerUserId != null) {
        ...
        vs =
            new ViewerStateVO(
                s != null && Boolean.TRUE.equals(s.getFavorite()),
                s != null && Boolean.TRUE.equals(s.getCompleted()),
                inLists.contains(r.getId()));
      }
      String sourceLabel = r.getSourceLabel() == null ? r.getSourceCode() : r.getSourceLabel();
      result.add(
          new ProblemListVO(
              r.getProblemNumber(),
              title(r.getProblemNumber(), r.getTitle()),
              r.getRegion(),
              r.getYear(),
              r.getSourceCode(),
              sourceLabel,
              r.getQuestionType(),
              type == null ? r.getQuestionType() : type.label(),
              r.getDifficulty(),
              tags.getOrDefault(r.getId(), List.of()),
              sourceText(r.getYear(), sourceLabel, r.getRegion()),
              r.getContent(),
              Optional.ofNullable(r.getContentFormat()).orElse("markdown-latex-v1"),
              Boolean.TRUE.equals(r.getDeleted()) ? List.of() : assets.getOrDefault(r.getId(), List.of()),
              new ProblemStatsVO(
                  Optional.ofNullable(r.getViewCount()).orElse(0L),
                  Optional.ofNullable(r.getFavoriteCount()).orElse(0L)),
              vs,
              curricula.getOrDefault(
                  r.getId(), cn.mathsea.backend.curriculum.CurriculumAnnotation.unknown())));
    }
    return result;
  }
  ...
  private void validateCodes(List<String> types, List<String> levels) {
    if (types.stream().anyMatch(v -> QuestionType.fromCode(v) == null))
      throw BusinessException.badRequest("INVALID_TYPE", "存在不合法的题型");
    if (levels.stream().anyMatch(v -> DifficultyLevel.fromCode(v) == null))
      throw BusinessException.badRequest("INVALID_LEVEL", "存在不合法的训练价值");
  }
  ...
  private String title(String number, String title) {
    return title == null || title.isBlank() ? number : title;
  }
