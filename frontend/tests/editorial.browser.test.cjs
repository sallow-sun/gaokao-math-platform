...
;(async () => {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (pathname.endsWith('/auth/me'))
      ...
      body = [
        {
          id: paperId,
          title: '2024全国甲卷',
          pending_count: Number(['DRAFT', 'REVIEW'].includes(item.status)),
          changes_count: Number(item.status === 'CHANGES'),
        },
        { id: 'finished', title: '已全部通过的试卷', pending_count: 0, changes_count: 0 },
        { id: 'changes', title: '只有待修改题目的试卷', pending_count: 0, changes_count: 1 },
      ]
      ...
      body = {
        items: matching
          ? [{ ...item, title: item.document.title, paper_title: '2024全国甲卷' }]
          : [],
        total: matching ? 1 : 0,
        page: 1,
        pageSize: 40,
  ...
  await page
    .getByRole('heading', { name: '题目工作台', exact: true })
  ...
  assert.deepEqual(
    await page.getByLabel('试卷', { exact: true }).locator('option').allTextContents(),
    ['全部试卷', '2024全国甲卷'],
  )
  ...
  assert.equal(
    await page.locator('.curriculum-editor').count(),
    0,
    'initial review hides classification',
  )
  assert.equal(await page.locator('.editorial-queue').count(), 0, 'directory starts collapsed')
  await page.getByRole('button', { name: /目录 ·/ }).click()
  assert.equal(
    await page.locator('.editorial-queue input[type="checkbox"]').count(),
    0,
    'review directory only opens individual questions',
  )
  ...
  assert.equal(
    await page.getByLabel('试卷', { exact: true }).locator('option[value="finished"]').count(),
    0,
  )
  assert.equal(
    await page.getByLabel('试卷', { exact: true }).locator('option[value="changes"]').count(),
    1,
  )
  await page.locator('.editorial-queue-row button').first().click()
  ...
  await page.getByLabel('试卷', { exact: true }).selectOption(paperId)
  await page.getByRole('button', { name: '通过并下一题', exact: true }).click()
  assert.equal(action, 'PUBLISH')
  await page
    .getByLabel('试卷', { exact: true })
    .locator(`option[value="${paperId}"]`)
    .waitFor({ state: 'detached' })
  assert.equal(await page.getByLabel('试卷', { exact: true }).inputValue(), '')
  ...
  console.log(
    'PASS: edit, preview, save, submit, publish, directory selection, responsive layout, no browser errors',
  )
})()
  .catch((error) => {
    console.error(error)
  ...
  .finally(async () => {
    ...
    if (
      path.resolve(fixture).startsWith(tempRoot + path.sep) &&
      path.basename(fixture).startsWith('mathsea-browser-')
    )
