...
export function usePracticeListPicker() {
  ...
  function createPracticeListFromPicker(formValue) {
  ...
  function applyPracticeListSelection() {
    ...
    if (pickerMode.value === 'manage') {
      ...
      practiceListsStore.lists.forEach((practiceList) => {
        ...
        if (shouldBeIncluded && !currentlyIncluded) {
          addedMembershipCount += practiceListsStore.addProblemsToPracticeList(practiceList.id, [
            problemId,
          ]).length
        } else if (!shouldBeIncluded && currentlyIncluded) {
          const removed = practiceListsStore.removeProblemFromPracticeList(
            practiceList.id,
            problemId,
          )
      ...
      selectedPracticeListIds.value.forEach((listId) => {
        addedMembershipCount += practiceListsStore.addProblemsToPracticeList(
          listId,
          pickerProblemIds.value,
        ).length
      })
