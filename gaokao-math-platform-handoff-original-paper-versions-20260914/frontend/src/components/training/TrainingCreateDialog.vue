      ...
      <div v-if="open" class="training-dialog-backdrop" @mousedown.self="cancel">
        ...
        >
          <header>
            ...
            <p id="training-create-description">登录后题单会保存到数据库，并可公开分享。</p>
          </header>
          ...
          <form @submit.prevent="submit">
            ...
            <label class="training-list-visibility-option">
              <input v-model="isPublic" type="checkbox" :disabled="isOfficial" />
              公开题单，允许其他用户查看和打印
            </label>
            <label v-if="allowOfficial" class="training-list-visibility-option">
              <input v-model="isOfficial" type="checkbox" @change="isPublic = true" />
              创建为官方题单
            </label>
