...
const aiLevelLabels = ['D1 · 基础', 'D2 · 较易', 'D3 · 中等基础', 'D4 · 中等', 'D5 · 较难', 'D6 · 困难', 'D7 · 高难']
const aiLevelOptions = DIFFICULTY_LEVELS.map((value, index) => ({
  value,
  label: aiLevelLabels[index],
}))
const statusLabels = {
  DRAFT: '初审',
  CHANGES: '待修改',
...
function applyAiUploadSuggestions(updates) {
  for (const field of ['title', 'content', 'answer', 'solution', 'type', 'level']) {
    if (Object.hasOwn(updates, field)) draft.value[field] = updates[field]
  }
  if (Array.isArray(updates.tags)) draft.value.tags = [...updates.tags]
  note.value = [note.value.trim(), '已人工确认 AI 辅助分类建议'].filter(Boolean).join('；')
}
            ...
            <div class="editorial-columns" :class="{ 'is-editing': editing }">
              ...
              <fieldset v-if="editing" class="editorial-form" :disabled="busy">
                <AiUploadAssistant
                  official
                  :draft="draft"
                  :level-options="aiLevelOptions"
                  :tag-options="props.tags"
                  :type-options="typeOptions"
                  @apply="applyAiUploadSuggestions"
                />
                <div class="editorial-actions">
                  <button
                    v-for="t in [
