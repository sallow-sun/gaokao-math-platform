...
import { refreshTagCatalog, tagCatalog } from '../services/tagCatalog.js'
import MathText from '../components/content/MathText.vue'
import AiUploadAssistant from '../features/ai-upload/AiUploadAssistant.vue'
import { inspectUploadDraft } from '../features/ai-upload/uploadRuleEngine.js'
...
const levelOptions = [
  ...
  { value: 'orange', label: 'D2 · 较易' },
  { value: 'yellow', label: 'D3 · 中等基础' },
  { value: 'green', label: 'D4 · 中等' },
  { value: 'cyan', label: 'D5 · 较难' },
  { value: 'blue', label: 'D6 · 困难' },
  { value: 'purple', label: 'D7 · 高难' },
]
const standardTags = computed(() => tagCatalog.value.map((item) => item.name))
...
function applyAiSuggestions(updates) {
...
async function submit() {
  ...
  const report = inspectUploadDraft(
    form.value,
    { types: typeOptions, levels: levelOptions, tags: standardTags.value },
    { official: false },
  )
  if (!report.valid) {
    error.value = `提交前请先修正：${report.errors.map((item) => item.title).join('；')}`
    return
  }
  busy.value = true
  try {
...
<template>
  <main class="community-upload">
    ...
    <section class="community-upload-card">
      ...
      <AiUploadAssistant
        v-if="user && !sent"
        class="community-ai-upload"
        :draft="form"
        :level-options="levelOptions"
        :tag-options="standardTags"
      ...
      <form v-else-if="user" @submit.prevent="submit">
        <fieldset :disabled="busy">
          ...
          <div v-if="form.level || form.tags.length" class="community-ai-classification">
            <strong>已采用的 AI 分类建议</strong>
            <span v-if="form.level">{{ levelOptions.find((item) => item.value === form.level)?.label }}</span>
            <span v-for="tag in form.tags" :key="tag">{{ tag }}</span>
            <button type="button" @click="Object.assign(form, { level: '', tags: [] })">
              清除分类建议
            </button>
          </div>
...
.community-ai-classification strong {
  margin-right: 4px;
}
.community-ai-classification span {
  padding: 4px 8px;
  color: #235f62;
  border-radius: 999px;
  background: white;
}
.community-ai-classification button {
  padding: 4px 8px;
  color: #725143;
  border: 1px solid #d5bbb0;
  background: white;
}
fieldset {
  border: 0;
