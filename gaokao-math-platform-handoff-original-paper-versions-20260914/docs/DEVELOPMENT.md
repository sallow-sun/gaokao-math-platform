...
仅做本机临时界面联调且没有 Redis 时，可用 `--spring.autoconfigure.exclude=org.springframework.boot.autoconfigure.session.SessionAutoConfiguration --mathsea.security.rate-limit-enabled=false` 启动后端。该组合使用 Servlet 容器的进程内会话并显式关闭限流，只能用于单机开发；生产环境保持默认值，必须使用 Redis。少数受限 Windows 环境无法建立 Java NIO 的内部回环连接，可另外传入 `--mathsea.local.tomcat-nio2=true` 切换 Tomcat 网络实现。
...
首次启动会执行 Flyway V1～V17，包括题库、审核、共享试卷、错题、用户成长及原卷版本表。不需要任何 SQLite 文件或生产数据库备份。已执行的迁移不能编辑，后续从 V18 开始新增迁移。
