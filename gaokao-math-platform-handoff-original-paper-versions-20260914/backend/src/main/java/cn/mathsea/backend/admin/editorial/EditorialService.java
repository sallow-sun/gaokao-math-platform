...
public class EditorialService {
  ...
  public record Contribution(String title, Integer year, String source, String type,
      String content, String answer, String solution, String level, List<String> tags) {
    public Contribution(String title, Integer year, String source, String type,
        String content, String answer, String solution) {
      this(title, year, source, type, content, answer, solution, null, List.of());
    }
  }
  ...
  public Object contribute(Long actor, Contribution input) {
    ...
    String level = Objects.toString(input.level(), "").trim();
    if (level.isEmpty()) level = "yellow";
    if (!MarkdownQuestionParser.LEVELS.contains(level))
      throw BusinessException.badRequest("CONTRIBUTION_INPUT", "AI 建议的难度不合法，请清除后重试");
    var doc = canonicalTags(new EditorialDocument(clean(input.title(), 255, "题目名称"), input.year(),
        clean(Objects.toString(input.source(), ""), 255, "来源"), input.type(), level,
